---
title: "Stan Workshop at Calvin College"
author: ""
date: "June 2019"
output:
  html_document:
...
the building, denoted below as `traps`. This model assumes that the conditional
mean and variance of the outcome variable `complaints` (number of complaints) is
the same. We'll investigate whether this is a good assumption after we fit the
model.
...
However, before we fit the model to real data, we should check that our model
works well with simulated data. We'll simulate data according to the model and
then check that we can sufficiently recover the parameter values used in the
simulation (ideally we would do this for many different draws from the prior).
You can think of this step as software testing.
...
```{r fit_model_P-fake-data, cache=TRUE, results="hide", message=FALSE}
...
Because we're not simulating so many observations, we shouldn't expect to
recover the parameters perfectly, but it looks at least _plausible_ ($\alpha$
and $\beta$ are contained within the histograms). If we did the simulation with
many more observations the parameters would be estimated much more precisely.
It's also a good idea to do this many times using different draws from the prior
to get a sense for the variety of the data that can be generated from the model.
...
package [vignettes](https://mc-stan.org/bayesplot/articles/graphical-ppcs.html)
...
(square roots of) expected counts vs the observed counts (blue histogram). We
...
The rootogram is sometimes a useful plot to compare the observed vs expected
number of complaints (on the square-root scale to make large differences easier
to plot). This is a plot of the square root of the expected counts (continuous
line and interval) vs the observed counts (blue histogram):
...
Adding in `live_in_super` gives us: 
$$
\eta_{b,t} = \alpha + \beta \, \textrm{traps}_{b,t} +
...
```{r comp_model_P_mult, message=FALSE, warning=FALSE}
...
```{r comp_dgp_multiple_NB, results="hide", message=FALSE}
...
```{r comp_model_NB, results="hide", message=FALSE}
...
```{r ppc-group_means, fig.width=7, fig.height=6}
ppc_stat_grouped(
  y = stan_dat_simple$complaints, 
  yrep = y_rep, 
...
```{r comp_model_NB_hier, results="hide", message=FALSE}
...
```{r fitted_model_NB_hier-low-ESS}
...
Looks like divergences, the little red bars underneath the traceplots
correspond to spots where sampler gets stuck at one parameter value for 
$\sigma_\mu$.
...
```{r fitted_model_NB_hier_ncp-ESS}
...
```{r}
library(ggformula)
gf_dist("beta", shape1 = 10, shape2 = 5)
```
